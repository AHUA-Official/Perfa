...
SCENARIOS = {
    "quick_test": Scenario(
        name="quick_test",
        display_name="快速测试",
        description="单项快速测试，如测一下CPU、跑个fio、sysbench 或 openssl speed",
        keywords=["快速", "测一下", "跑个", "单项", "简单测试", "quick", "sysbench", "openssl", "stress-ng", "iperf3", "7z"]
    ),
    "full_assessment": Scenario(
        name="full_assessment",
        display_name="全面性能评估",
        description="全面性能评估，默认走短测链路，包含CPU/内存/磁盘/网络全项测试",
        keywords=["全面", "完整", "综合", "评估", "全量", "full", "all"]
    ),
    "cpu_focus": Scenario(
        name="cpu_focus",
        display_name="CPU 专项评估",
        description="CPU 专项评估，默认走 sysbench + OpenSSL 短测；显式指定 unixbench/superpi 时再跑长测",
        keywords=["cpu", "处理器", "算力", "计算性能", "unixbench", "superpi", "sysbench", "openssl"]
    ),
    "storage_focus": Scenario(
        name="storage_focus",
        display_name="存储专项评估",
        description="存储/内存测试，默认走 sysbench memory + FIO；显式指定 mlc/stream 时再跑对应测试",
        keywords=["磁盘", "硬盘", "存储", "io", "内存带宽", "fio", "mlc", "stream", "disk", "sysbench memory"]
    ),
    "network_focus": Scenario(
        name="network_focus",
        display_name="网络专项评估",
        description="网络测试，默认走 iperf3 + hping3 短测链路",
        keywords=["网络", "延迟", "带宽", "丢包", "hping3", "network", "ping", "iperf3", "吞吐"]
    ),
    "free_chat": Scenario(
        name="free_chat",
        display_name="自由对话",
...
- quick_test: 单项快速测试，如"测一下CPU"、"跑个fio"、"帮我跑个superpi"、"跑个sysbench cpu"
- full_assessment: 全面性能评估，如"全面评估服务器性能"、"完整测试"、"综合评估"，默认走短测
- cpu_focus: CPU 专项测试，如"详细CPU测试"、"CPU性能分析"、"测试处理器"，默认走短测
- storage_focus: 存储/内存测试，如"测试磁盘性能"、"内存带宽测试"、"IO性能"，默认走短测
- network_focus: 网络测试，如"测试网络延迟"、"网络质量"、"ping测试"，默认走短测
